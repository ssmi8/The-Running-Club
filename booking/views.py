...
runs_total = RunName.objects.filter(booking__status='Available').count()
...
def check_availabilty(customer_run_name, customer_requested_date):
...
class BookingEnquiry(View):
    ...
    def post(self, request):
        ...
        if customer_form.is_valid() and booking_form.is_valid():
            ...
            runs_booked = check_availabilty(
                customer_run_name, date_formatted)
            ...
            if runs_booked > booking_runs:
                ...
                messages.add_message(
                    ...
                    "Unfortunately," f"{customer_run_name} " "\
                        is fully booked on "
